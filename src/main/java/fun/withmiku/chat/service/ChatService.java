...
import fun.withmiku.chat.ai.guard.EmotionalBoundaryGuard;
...
public class ChatService {
     ...
     * [ system prompt ]
     * [ （可选）boundary mode prompt ]
     * [ （可选）runtime guard message ]
     * [ history messages... ]
     */
    public String chat(String userInput) {
        // 1. 构造本次请求的 messages（不是直接用 memory）
        List<Message> messagesToSend = new ArrayList<>();

        // 2. 如果检测到危险情感，向本次请求的 messages 插入“临时人格约束”（不进入 memory）
        if (EmotionalBoundaryGuard.isDependencyRisk(userInput)) {
            String pattern = EmotionalBoundaryGuard.matchedPattern(userInput);
            ...
            if (pattern != null) {
                ...
                log.warn(
                        "[EmotionBoundaryGuard HIT] pattern='{}', userInput='{}'",
                        pattern,
                        // 用户输入缩略
            ...
            messagesToSend.add(EmotionalBoundaryGuard.guardMessage());
        }

        // 3. 用户输入进入 memory
        memory.add(new Message("user", userInput));

        // 4.1 永久 system prompt（不进 memory）
        messagesToSend.add(new Message("system", MikuPrompt.SYSTEM_PROMPT));

        // 4.2 运行时人格护栏（仅在检测到越权时）
